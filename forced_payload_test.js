    ...
    for(var i = 0; i<results.length; i++) {
      ...
      try {
        ...
        if(payload.lines.length > 0) {
          ...
          //For testing purposes hard coding 400 res code
          var resCode = 400;
          var resString = "I'm not a real response. Just a test to not break the main try catch"
          ...
          //This is my test else case just to see if I can place the payload in the cxml
          else{

            //Forcing the record to be updated.
            nlapiSubmitRecord(rec);
            nlapiLogExecution('AUDIT','SUCCESS','CXML Updated');
          }
  
          //else if errors are present, log them and email jacobg@overturepromo.com or kevind@overturepromo.com
          //The response variable is no present so this will just break the main try catch and never submit the record.
        //   else {
